...
type Resolved = { status: 'idle' } | { status: 'loading' } | { status: 'done'; address: string | null }
...
export function AddressPicker() {
  ...
  return (
    <div className="card stack">
      <h2 style={{ fontSize: '1.2rem' }}>Mi dirección</h2>
      ...
      <form className="stack" onSubmit={onSubmit}>
        ...
        <div className="row">
          <button type="submit" className="btn" disabled={!dirty || save.pending}>
            {save.pending ? 'Guardando…' : current.data ? 'Guardar cambios' : 'Guardar dirección'}
          </button>
          {current.data && (
            <button
              type="button"
