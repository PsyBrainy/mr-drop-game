...
def prev(names, path, k=2):
    ...
    for n in names:
        ...
        for i,f in enumerate(fr):
            ...
            d=ImageDraw.Draw(b)
            # hurtbox
            d.rectangle([(OX-16)*2,(OY-56)*2,(OX+16)*2-1,OY*2-1],outline=(80,200,255))
            d.line([0,OY*2,F,OY*2],fill=(0,0,0))
            if i in hitposes:
                (x0,x1),(y0,y1),_=BOX[n]
                d.rectangle([(OX+x0)*2,(OY-y1)*2,(OX+x1)*2-1,(OY-y0)*2-1],outline=(255,60,60))
            tiles.append(b)
        rows.append(tiles)
    ...
    s.resize((s.width*k//2,s.height*k//2),Image.NEAREST).save(path)
